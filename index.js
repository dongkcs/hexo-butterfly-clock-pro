'use strict'
// 全局声明插件代号
const pluginname = 'electric_clock_pro'
// 全局声明依赖
...
hexo.extend.filter.register('after_generate', function (locals) {
  // 首先获取整体的配置项名称
  const config = hexo.config.electric_clock_pro ? hexo.config.electric_clock_pro : hexo.theme.config.electric_clock_pro
  // 如果配置开启
  ...
  // 集体声明配置项
    const data = {
      ...
      clock_js: config.clock_js ? urlFor(config.clock_js) : "lib/clock.js",
...
hexo.extend.helper.register('priority', function(){
  ...
  const pre_priority = hexo.config.electric_clock_pro.priority ?  hexo.config.electric_clock_pro.priority : hexo.theme.config.electric_clock_pro.priority
